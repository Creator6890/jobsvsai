...
ALTER TABLE task_mapping_rubric_versions
  ADD COLUMN IF NOT EXISTS minimum_meaningful_constraint_level NUMERIC(6,3) NOT NULL DEFAULT 10
  CHECK (minimum_meaningful_constraint_level BETWEEN 0 AND 100);
...
BEGIN
  ...
  WHERE item.gold_dataset_id=dataset_key AND (
    ...
    OR capability_summary.confidence_errors>0 OR constraint_summary.threshold_errors>0
    OR constraint_summary.version_errors>0 OR constraint_summary.confidence_errors>0
  );
